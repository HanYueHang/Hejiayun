<template>
  <div>
    ...
    <a-row class="header">
      ...
      <a-select v-model="form2.region" @change="changeRegion()">
...
export default {
    ...
    created() {
        ...
        const param = QS.stringify(sendDate)
        selectBuilding(param).then(res => {
    ...
    methods: {
        ...
        nextStep() {
            const buildingData = this.data
            let param = '['
            for (let i = 0; i < buildingData.length; i++) {
                if (i !== buildingData.length - 1) {
                    param = param + '{"buildingCode": "' + buildingData[i].buildingCode + '", "unitCount": ' + buildingData[i].unitCount + '},'
                } else {
                    param = param + '{"buildingCode":"' + buildingData[i].buildingCode + '", "unitCount": ' + buildingData[i].unitCount + '}'
                }
            }
